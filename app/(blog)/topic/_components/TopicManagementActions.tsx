...
interface TopicManagementActionsProps {
  ...
  return (
    <div className="flex flex-wrap items-center justify-end gap-3 mb-6 animate-in fade-in slide-in-from-right-2 duration-300">
      ...
      >
        ...
        <span>
          {loading ? "Wait" : isCurrentlyActive ? "Hide" : "Activate"}
        </span>
      </button>
      ...
      {isModalOpen && (
        <TopicEditModal topic={topic} onClose={() => setIsModalOpen(false)} />
      )}
    </div>
  );
};

export default TopicManagementActions;
